...
### 新增功能
- 💾 **SQLite 設定儲存**：所有設定現在都會保存到 SQLite 資料庫中
  - 創建 `utils/settings_db.py`：設定資料庫管理模組
  - 資料庫文件位置：Windows 在 `%APPDATA%\RelaxTime\settings.db`，其他系統在 `~/.relaxtime/settings.db`
  - 保存的設定包括：
    - `default_duration`：預設工作時間（分鐘）
    - `rest_duration`：休息時間（分鐘）
    - `loop_mode`：循環模式（布林值）
    - `startup_enabled`：開機自動啟動（布林值）
  - 應用程式啟動時自動從資料庫載入設定
  - 設定改變時自動保存到資料庫
  - 更新 `controllers/timer_controller.py`：整合資料庫讀寫功能
  - 更新 `utils/__init__.py`：導出 SettingsDB 類

## 2026-01-02 09:45:21

### UI 改進
- 📐 **調整主視窗大小和布局**：將主視窗大小改為 300x300 像素，所有內容置中顯示
  - 視窗大小從 400x500 調整為 300x300
  - 所有 UI 元素（時間顯示、狀態標籤、按鈕等）都置中對齊
  - 調整字體大小和間距以適應更小的視窗
  - 使用 grid 布局的 sticky 參數實現居中效果
  - 更新 `views/main_window.py`：調整視窗大小和 UI 布局

## 2026-01-02 09:42:25

### UI 改進
- 📐 **固定主視窗大小**：將主視窗大小固定為 400x500 像素
  - 無論視窗是隱藏還是顯示，都保持相同的大小
  - 改進視窗顯示的一致性，提供更好的用戶體驗
  - 更新 `views/main_window.py`：添加固定視窗大小屬性，確保在顯示和隱藏時都保持 400x500 大小
